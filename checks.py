...
class checks:
    ...
    def __init__(self, agentConfig, rawConfig, mainLogger):
        ...
        # Add header
        headers.update({'Authorization':"Bearer " + self.agentConfig['agentKey']})
    ...
    def getApacheStatus(self):
        ...
        if 'apacheStatusUrl' in self.agentConfig and self.agentConfig['apacheStatusUrl'] != 'http://example.com/server-status/?auto':    # Don't do it if the status URL hasn't been provided
    ...
    def doChecks(self, sc, firstRun, systemStats=False):
        ...
        if diskUsage:
            checksData['disk_usage'] = diskUsage

        if loadAvrgs:
            checksData['load_avrg'] = loadAvrgs['1']

        if memory:
            checksData['mem_phys_used'] = memory['physUsed']
            checksData['mem_phys_free'] = memory['physFree']
            checksData['mem_swap_used'] = memory['swapUsed']
            checksData['mem_swap_free'] = memory['swapFree']
            checksData['mem_cached'] = memory['cached']

        if networkTraffic:
            checksData['network_traffic'] = networkTraffic
