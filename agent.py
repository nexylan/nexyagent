...
try:
    ...
    if os.path.exists('/etc/nexyagent/conf.d/'):
        configPath = '/etc/nexyagent/conf.d/'
...
if __name__ == '__main__':
    ...
    if argLen == 3 or argLen == 4: # needs to accept case when --clean is passed
        if sys.argv[2] == 'init':
            ...
            # a separate nexyagent user account.
        ...
        if 'start' == sys.argv[1]:
            ...
            if updateInfo['version'] != agentConfig['version']:
                ...
                def downloadFile(agentFile, recursed = False):
                    ...
                    downloadedFile = urllib.urlretrieve('http://www.serverdensity.com/downloads/nexyagent/' + agentFile['name'])
